...
Jobs checkpoint each successful stage, reclaim expired leases, and stop after three attempts. Inspect `osa jobs` for failures; fix inputs/model before submitting a new job. Live checkout creation is not automatically retried if its outcome is uncertain; inspect the provider using the recorded order `reference_id` first. SMTP rows left `sending` or `uncertain` require provider review before manual retry to avoid duplicate messages.
